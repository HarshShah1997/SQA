...
from parsy.dsl import IsRelatedTo
from parsy.expression import isnode
from parsy.encodingpolicy import assert_valid_encoding
...
def expression_to_sparql(e, full=False):
    ...
    value = xs[-1][-2]
    xs.append('FILTER(!isLiteral(?x' + value + ') || langMatches(lang(?x' + value + '), "EN") || lang(?x' + value + ') = "")')
